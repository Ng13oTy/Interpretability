...
class Att_BLSTM(nn.Module):
    def __init__(self, word2vec, node_type_dic, max_sen_len, conv_layer_info, att_layer_info):
        super(Att_BLSTM, self).__init__()
        self.hidden_size = att_layer_info[0]
        ...
        # 注意力层
        self.lstm = nn.LSTM(
            input_size=self.word_embed_size + len(node_type_dic),
            hidden_size=att_layer_info[0],
            bidirectional=True,
            batch_first=True
        )
        self.tanh = nn.Tanh()
        self.att_weight = nn.Parameter(torch.Tensor(att_layer_info[0], 1))
        self._create_weights(mean=0.0, std=0.05)

        # 线性分类
        self.classifier = nn.Linear(in_features=att_layer_info[0], out_features=1)
        self.sigmoid = nn.Sigmoid()

    def _create_weights(self, mean=0.0, std=0.05):
        self.att_weight.data.normal_(mean, std)
    ...
    def forward(self, batch, if_count_hit=False, if_cwe=False):
        ...
        # 进入注意力层
        lstm_output, (_, _) = self.lstm(sen_input)
        # 该模型是将BLSTM的正反向输出取和，而不是拼接
        lstm_output = lstm_output.view(-1, self.max_sen_len, 2, self.hidden_size)
        lstm_output = torch.sum(lstm_output, dim=2)

        t0_output = self.tanh(lstm_output)
        output = matrix_mul(t0_output, self.att_weight)
        ...
        # print(output.shape)
        output = element_wise_mul(lstm_output, output)
