...
class GraphWithHAN(nn.Module):
    def __init__(self, word2vec, node_type_dic, max_sen_len, conv_layer_info, sen_att_layer_info, word_att_layer_info):
        super(GraphWithHAN, self).__init__()
        ...
        # 单词注意力层
        self.word_weight = nn.Parameter(torch.Tensor(2 * word_att_layer_info[0], 2 * word_att_layer_info[0]))
        self.word_bias = nn.Parameter(torch.Tensor(1, 2 * word_att_layer_info[0]))
        self.word_context_weight = nn.Parameter(torch.Tensor(2 * word_att_layer_info[0], 1))
        self.word_gru = nn.GRU(self.word_embed_size, word_att_layer_info[0], bidirectional=True,
                               batch_first=True)
        # 句子注意力层
        ...
        self.sen_context_weight = nn.Parameter(torch.Tensor(2 * sen_att_layer_info[0], 1))
        self.sen_gru = nn.GRU(2 * word_att_layer_info[0] + len(self.node_type_dic), sen_att_layer_info[0],
    ...
    def _create_weights(self, mean=0.0, std=0.05):
        ...
        self.sen_context_weight.data.normal_(mean, std)
    ...
    def forward(self, batch, if_count_hit=False, if_cwe=False):
        ...
        for g in batch:
            ...
            # 得到单词级注意力
            w_gru_output, _ = self.word_gru(nodes_tensor)
            w_output = matrix_mul(w_gru_output, self.word_weight, self.word_bias)
            w_output = matrix_mul(w_output, self.word_context_weight)
            w_output = F.softmax(w_output, dim=1)
            w_output = element_wise_mul(w_gru_output, w_output)
            ...
            g_info = torch.cat((w_output, n_type_one_hot), dim=1)
        ...
        # 进入句子级注意力层
        s_gru_output, _ = self.sen_gru(sen_input)
        # print(gru_output.shape)
        output = matrix_mul(s_gru_output, self.sent_weight, self.sent_bias)
        # print(output.shape)
        output = matrix_mul(output, self.sen_context_weight)
        ...
        # print(output.shape)
        output = element_wise_mul(s_gru_output, output)
        # print(output.shape)
